# Hi, I'm Shruti 👋

**Final-year B.Tech CSE @ NIT Patna** (CGPA: 8.19) · Software Engineering Intern @ **IBM** · Seeking SDE / AI Engineer roles

📍 India · 📧 shruti26205@gmail.com · 📱 8544131434

---

## 🚀 About Me

I build backend systems that scale — clean APIs, solid data layers, and AI-powered features. Currently interning at IBM working on enterprise storage management for IBM FlashSystem, while building AI engineering skills on the side.

- 🔭 Currently building: **RAG Document Q&A** — Spring Boot + Gemini AI + pgvector
- 🌱 Levelling up in: AI Engineering (RAG, vector databases, Spring AI, LLM APIs)
- 💼 Open to: **SDE / AI Engineer** roles at product companies
- 🎓 Graduating: **June 2026** · Available immediately after

---

## 🛠 Tech Stack

**Languages**
![Java](https://img.shields.io/badge/Java-ED8B00?style=flat&logo=openjdk&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-4479A1?style=flat&logo=mysql&logoColor=white)
![C](https://img.shields.io/badge/C-00599C?style=flat&logo=c&logoColor=white)

**Backend**
![Spring Boot](https://img.shields.io/badge/Spring_Boot-6DB33F?style=flat&logo=spring-boot&logoColor=white)
![Spring Security](https://img.shields.io/badge/Spring_Security-6DB33F?style=flat&logo=spring&logoColor=white)
![Hibernate](https://img.shields.io/badge/Hibernate-59666C?style=flat&logo=hibernate&logoColor=white)
![REST APIs](https://img.shields.io/badge/REST_APIs-009688?style=flat)

**AI / ML**
![Spring AI](https://img.shields.io/badge/Spring_AI-6DB33F?style=flat&logo=spring&logoColor=white)
![Gemini](https://img.shields.io/badge/Gemini_API-4285F4?style=flat&logo=google&logoColor=white)
![pgvector](https://img.shields.io/badge/pgvector-336791?style=flat&logo=postgresql&logoColor=white)

**Infrastructure & Tools**
![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat&logo=docker&logoColor=white)
![Redis](https://img.shields.io/badge/Redis-DC382D?style=flat&logo=redis&logoColor=white)
![MySQL](https://img.shields.io/badge/MySQL-4479A1?style=flat&logo=mysql&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-336791?style=flat&logo=postgresql&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=flat&logo=git&logoColor=white)
![JWT](https://img.shields.io/badge/JWT-000000?style=flat&logo=jsonwebtokens&logoColor=white)

---

## 💼 Experience

### Intern — IBM *(Jan 2026 – Present)*
- Gained hands-on experience with enterprise-scale distributed storage 
  systems, Linux internals, and production infrastructure at IBM scale
- Developed technical depth in system diagnostics, failure analysis, 
  and SLA-driven operations in high-availability environments
- Collaborated with L3 engineering teams on critical production issues


---

## 🔨 Projects

### 🤖 [RAG Document Q&A](https://github.com/shru42/rag-doc-qa)
AI-powered system that answers questions about any PDF — answers grounded in the document, not hallucinated.
- **Spring Boot 3.5 + Spring AI 1.0 + Gemini API** (gemini-embedding-001, 3072-dim vectors)
- pgvector semantic search · Redis response caching · Full Docker setup
- Complete RAG pipeline: PDF → chunk → embed → vector search → grounded answer

### 🔗 [URL Shortener](https://github.com/shru42/url-shortener)
Scalable URL shortening service capable of generating ~56 billion unique short URLs.
- **Java · Spring Boot · MySQL · Redis · Docker Compose**
- Redis cache-aside pattern → 70% reduction in DB load, <10ms average redirect latency
- Base62 encoding with collision-free guarantee

### 🎫 [IT Incident Tracker](https://github.com/shru42/it-incident-tracker)
Production-grade REST API for IT incident management — live on Railway.
- **Java · Spring Boot · MySQL · JWT · Spring Security · RBAC**
- 3 user roles (Admin, Engineer, Viewer) · BCrypt password hashing
- 35% query response time improvement via strategic indexing · Deployed on Railway

---

## 📊 Competitive Programming


- 🏆 **Rank 301 / 6,499** in CodeClash 2025 — top 5%
- ⭐ **LeetCode rating 1750** · Global rank 88,726 / 874,367 · Top 10.37% · 36 contests
- 💡 **800+ problems** solved across LeetCode & Codeforces
- 🎯 Strong in: Graph algorithms, Dynamic Programming, Trees, Sliding Window
- 🎓 **NTSE State-Level** qualifier — top percentile statewide

[![LeetCode](https://img.shields.io/badge/LeetCode-FFA116?style=flat&logo=leetcode&logoColor=white)](https://leetcode.com/u/smayashruti28830/)
[![Codeforces](https://img.shields.io/badge/Codeforces-1F8ACB?style=flat&logo=codeforces&logoColor=white)](https://codeforces.com/profile/shruti_3)
[![GFG](https://img.shields.io/badge/GeeksForGeeks-2F8D46?style=flat&logo=geeksforgeeks&logoColor=white)]([https://geeksforgeeks.org](https://www.geeksforgeeks.org/profile/smayashruy7ut?tab=activity))

---

## 🎓 Education

**B.Tech Computer Science Engineering** · NIT Patna · Nov 2022 – Jun 2026 · **CGPA: 8.19/10**

---

## 📫 Let's connect

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=flat&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/shruti-pandey-3591a3265/)
[![GitHub](https://img.shields.io/badge/GitHub-100000?style=flat&logo=github&logoColor=white)](https://github.com/shru42)
[![Email](https://img.shields.io/badge/Gmail-D14836?style=flat&logo=gmail&logoColor=white)](mailto:shruti26205@gmail.com)

---

*💡 Open to SDE / AI Engineer roles — graduating June 2026, available immediately*
